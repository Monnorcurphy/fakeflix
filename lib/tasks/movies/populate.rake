namespace :movies do
  ...
  top_246.each do |film|
    ...
    search = film.gsub(/ /, '+')
    movie = HTTParty.get('http://www.omdbapi.com/?', query:{'t': film, 'apikey': 'a458a04'})
    ...
    movie["Actors"].split(',').each do |actor|
      actor = actor.split(' ')
      i = 0
      while actor[i] == ' ' || actor[i] == ''
        i += 1
      end

      actor = actor.slice(i, actor.length)
      actor = actor.join(' ').split(' ')
      actor.each_with_index do |item, idx|
        item.downcase!
        item.capitalize!
      end
      actor = actor.join(' ')
      actor = actor.remove_non_ascii
      actors_url[actor] = HTTParty.get("http://api.tmdb.org/3/search/person?api_key=ddb0be799ce01cc0b22e6a5045353585&query=`#{actor}'")

    end

    packaged_movies = PackageMovies.new(movie, movie_url, actors_url)
    packaged_movies.create_movie
    packaged_movies.create_actor
  end
    end_time = Time.now
    duration = (start_time - end_time) / 1.minute
    log.info "Task finished at #{end_time} and lasted #{duration} minutes. Good to know"
    log.close
  end
end
